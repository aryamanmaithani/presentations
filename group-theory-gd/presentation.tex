...
\date[20-07-2020]{20th July 2020}
...
\begin{frame}{Some non-examples of groups}
    \begin{itemize}
        ...
        \uncover<8->{\item My friend group.}
    \end{itemize}
\end{frame}
\begin{frame}{What is a group?}
    ...
    \end{itemize}}
    \uncover<11->{Now, we define what a group is.}
\end{frame}
\begin{frame}{What is a group?}
...
\begin{frame}{Review}
    ...
    \uncover<2->{\begin{itemize}
        ...
        \uncover<8->{\item My friend group.}
    \end{itemize}}
    \uncover<9->{The last case is nothing special. Indeed, in general we need the set to be nonempty since it must always have the identity.} \\~\\
...
\begin{frame}{Abelian groups}
    ...
    \uncover<4->{\begin{defn}[Abelian groups]
        ...
        for all $a, b \in G.$
    \end{defn}}
    \uncover<5->{From the first slide, everything except for the matrix example was an example of an abelian.}\\
...
\begin{frame}{Notations}
    ...
    $x^0 \vcentcolon= e.$\\
    For $n > 0,$ we write $x^n$ to mean
    \begin{equation*} 
        \underbrace{x \cdot x \cdots  x}_{n \text{ times}}.
    \end{equation*}
    For $n < 0,$ we have $x^n := (x^{-1})^{-n},$ which is the same as $(x^{-n})^{-1}.$ (Prove!)\\~\\
...
\begin{frame}{Examples}
    ...
    \begin{itemize}
        ...
        \uncover<7->{\item The set of $n \times n$ upper invertible diagonal (real) matrices is a subgroup of the group of all invertible $n\times n$ (real) matrices. }
    \end{itemize}
\end{frame}
\begin{frame}{Cosets}
    ...
    \end{equation*} }\\
    \uncover<3->{\begin{defn}[Coset]
        A \emph{(left) coset} of $H$ is a set of the form $g\cdot H.$
    \end{defn} }
    \uncover<4->{Define $G/H$ be the set of cosets, that is,
    \begin{equation*} 
        G/H  \vcentcolon= \{gH : g \in G\}.
    \end{equation*} }
    \uncover<5->{Note that different elements could correspond to the same coset. }\uncover<6->{In fact, we now see precisely when that is possible. }
\end{frame}
\begin{frame}{Properties of cosets}
    \begin{prop}[Equality of cosets]
        Let $a, b \in G.$ Then, 
        \begin{equation*} 
            aH = bH \text{ iff } b^{-1}a \in H.
        \end{equation*}
    \end{prop}
...
\begin{frame}{Homomorphisms}
    ...
    \uncover<6->{The case of groups is particularly simple since there's pretty one much thing that gives the group its structure, the operation $\cdot$. }\\
...
\begin{frame}{Isomorphisms}
    \begin{defn}[Isomorphism]
        Let $G$ and $H$ be groups. A group homomorphism $\varphi:G \to H$ is said to be \emph{isomorphism} if $\varphi$ is bijective.
    \end{defn}
...
\begin{frame}{A curious property about kernels}
    ...
    \uncover<5->{This can be written in yet another way as $aK = Ka.$ }
\end{frame}
\begin{frame}{A curious calculation}
    ...
    \uncover<11->{Note that the only property we used was that $gK = Kg$ and not really that $K$ was a kernel. }
\end{frame}
